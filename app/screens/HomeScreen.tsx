...
const groupForecastToDays = (list: any[]): ForecastItem[] => {
  ...
  // convert to ForecastItem[] and keep next 5 days
  ...
  const fetchWeather = useCallback(
    async (searchCity: string) => {
      ...
      try {
        ...
        if (currentResp.status !== 200) throw new Error('City not found or API error (current)');
        const currentJson = currentResp.data;
        
        const forecastResp = await weatherService.getWeatherForcast(searchCity);
        if (forecastResp.status !== 200) throw new Error('City not found or API error (forecast)');
